...
```
Observation:
+ Same as Phase 3, there are 2 inputs (0x8(%rsp) and 0xc(%rsp). We name them as a and b)
+ a <= 0xe (14)
+ Key is to visit func4 and set $eax to 0
+ 0xc(%rsp) (b) have to be 0x0 (0)
Function 4:
``` assembly
0000000000400fce <func4>:
  ...
  40100b:	c3                   	retq   
```
On *0x400ff2, set eax to zero. Guess: Key is to jump to this code. On *0x400ff0, shown that it would jump to func4 + 0x39. So we should JMP to here. How?
``` assembly
  400fe2:	39 f9                	cmp    %edi,%ecx
  400fe4:	7e 0c                	jle    400ff2 <func4+0x24>
```
So we have %ecx <= %edi. When will the function ends? We should jump *0x400ffe (callq func4). So we have %ecx >= %edi. Thus: %ecx == %edi. From Phase 4 we know, %edi = 0x8(%rsp) = a. Using GDB, we could get that %ecx = 7. So the answer is: 7 0 (a=7, b=0) 
...
```
Observation:
+ Also a string, length is 0x6 (6)
+ Debug at *0x40108b, get that %rbx is the element we input
...
``` C
for(int i = 0 ; i < 6; ++i){
  arr[i] = mp[input[i] & 0xf]
}
```
Use GDB to see what's in the map:
```
...
0x40245e:       "flyers"
```
It's flyers. So we could get that, arr should be "flyers". Thus, we have:
+ input[0] & 0xf = 9 (f)
+ input[1] & 0xf = 15 (l)
+ input[2] & 0xf = 14 (y)
+ input[3] & 0xf = 5 (e)
...
Found that there is a LOOP: (From 0x400110e to 0x401153)
``` assembly
  40110e:	41 bc 00 00 00 00    	mov    $0x0,%r12d
  401114:	4c 89 ed             	mov    %r13,%rbp
  401117:	41 8b 45 00          	mov    0x0(%r13),%eax
...
```
So there are 6 distinct numbers no larger than 6.
``` assembly
  401153:	48 8d 74 24 18       	lea    0x18(%rsp),%rsi
  401158:	4c 89 f0             	mov    %r14,%rax
  40115b:	b9 07 00 00 00       	mov    $0x7,%ecx
  ...
  40116d:	75 f1                	jne    401160 <phase_6+0x6c>
```
Another loop. Here %rsi is (0x18)%rsp, which is also the end of the array.
``` C
for(int i = 0 ; i  < 6 ;++i){
  arr[i] = 7 - arr[i];
}
```
Then there's further operation
``` assembly
  40116f:	be 00 00 00 00       	mov    $0x0,%esi
  401174:	eb 21                	jmp    401197 <phase_6+0xa3>
  401176:	48 8b 52 08          	mov    0x8(%rdx),%rdx
...
```
We could get a new arry (narray), begin at %rsp+0x20, end at %rsp + 0x50. Using gdb, we get the data saved in *(add+8):
``` 
  (gdb) x 0x6032d0+0x8
...
``` assembly
  ...
  4011d2:	48 c7 42 08 00 00 00 	movq   $0x0,0x8(%rdx)
```
Here do operation on the new array.
``` C
for(int i = 1 ; i < 6 ; ++i){
  *(narr[i-1] + 8) = narr[i];
}
...
``` assembly
  ...
  4011f5:	75 e8                	jne    4011df <phase_6+0xeb>
```
We could get that eax is \*(narr[0]+0x8), which is narr[1], also \*(narr[1]). and (%rbx) is \*(narr[0]). Using gdb to get the number in narr and *(narr):
```
...
Thus, 
``` C
narr = {0x6032d0,0x6032e0,0x6032f0,0x603300,0x603310,0x603320};
*narr = {332,168,924,691,477,443}.
```
*narr should be in order, so it should be:
``` C
